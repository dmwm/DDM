...
Aview = FOREACH A GENERATE (((long) start_time) * 1000) as ots, (((long) end_time) * 1000) as cts, file_lfn, client_host, server_username, ((long)end_time - (long)start_time) as procTime, (long)read_bytes_at_close as readBytes;
Aview1 = FOREACH Aview GENERATE CONCAT ( (chararray)GetYear(ToDate(cts)), CONCAT('/', CONCAT( (chararray)GetMonth(ToDate(cts)), CONCAT('/', (chararray)GetDay(ToDate(cts)) ) ) ) ) as ctsYYMMDD, cts, file_lfn, client_host, server_username, procTime, readBytes;
AviewWhere = FILTER Aview1 BY procTime > 0;
...
groupby = GROUP view BY (cts, client_host, server_username, dsname);
