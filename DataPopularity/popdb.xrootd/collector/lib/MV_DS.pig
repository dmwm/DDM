...
-- This second part is copy&paste of MV_XRD_DS_stat0_aggr1
...
MV_XRD_DS_stat0_aggr1 = FOREACH aggr1 GENERATE FLATTEN(group), SUM(select.readBytes) as readBytes, SUM(select.numAccesses) as numAccesses, SUM(select.procTime) as totCPU, COUNT(select.server_username) as numUsers;

-- End of MV_XRD_DS_stat0_aggr1

DS = FOREACH MV_XRD_DS_stat0_aggr1 GENERATE collName;
DSGroupBy = GROUP DS BY (collName);
MV_DS = FOREACH DSGroupBy GENERATE FLATTEN(group);
DSOrderBy = ORDER MV_DS BY $0 ASC;

STORE DSOrderBy into '$output' USING PigStorage(','); 
