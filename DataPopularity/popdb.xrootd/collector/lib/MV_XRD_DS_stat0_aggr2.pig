...
select = FOREACH MV_XRD_stat0 GENERATE ctsYYMMDD, SUBSTRING(dsname, LAST_INDEX_OF(dsname, '/') + 1, 999) as collName, (REGEX_EXTRACT(server_username, '^cms', 0) == '' ? 0 : 1) as isUSERCMS, readBytes, numAccesses, procTime, server_username;
aggr2  = GROUP select BY (ctsYYMMDD, collName, isUSERCMS);
result = FOREACH aggr2 GENERATE FLATTEN(group), SUM(select.readBytes) as readBytes, SUM(select.numAccesses) as numAccesses, SUM(select.procTime) as totCPU, COUNT(select.server_username) as numUsers;
