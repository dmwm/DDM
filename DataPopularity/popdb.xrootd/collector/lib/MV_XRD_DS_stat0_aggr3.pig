...
select = FOREACH MV_XRD_stat0 GENERATE ctsYYMMDD, server_username, SUBSTRING(dsname, LAST_INDEX_OF(dsname, '/') + 1, 999) as collName, readBytes, numAccesses, procTime;
aggr1  = GROUP select BY (ctsYYMMDD, server_username, collName);
result = FOREACH aggr1 GENERATE FLATTEN(group), SUM(select.readBytes) as readBytes, SUM(select.numAccesses) as numAccesses, SUM(select.procTime) as totCPU;
