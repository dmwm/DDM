...
-- This second part is copy&paste of MV_XRD_DS_stat0_aggr2

select = FOREACH MV_XRD_stat0 GENERATE ctsYYMMDD, SUBSTRING(dsname, INDEXOF(dsname, '/') + 1, 999) as collName, (REGEX_EXTRACT(server_username, '^cms', 0) == '' ? 0 : 1) as isUSERCMS, readBytes, numAccesses, procTime, server_username;
aggr2  = GROUP select BY (ctsYYMMDD, collName, isUSERCMS);
MV_XRD_DS_stat0_aggr2 = FOREACH aggr2 GENERATE FLATTEN(group), SUM(select.readBytes) as readBytes, SUM(select.numAccesses) as numAccesses, SUM(select.procTime) as totCPU, COUNT(select.server_username) as numUsers;

-- End of MV_XRD_DS_stat0_aggr2

DataTier = FOREACH MV_XRD_DS_stat0_aggr2 GENERATE collName;
DataTierGroupBy = GROUP DataTier BY (collName);
MV_DataTier = FOREACH DataTierGroupBy GENERATE FLATTEN(group);
DataTierOrderBy = ORDER MV_DataTier BY $0 ASC;

STORE DataTierOrderBy into '$output' USING PigStorage(',');
