...
selectWhere = FILTER MV_XRD_stat0_pre BY DaysBetween(CurrentTime(), ToDate(ctsYYMMDD, 'yyyy/MM/dd')) < 180;
select = FOREACH selectWhere GENERATE GetWeek(ToDate(ctsYYMMDD, 'yyyy/MM/dd')) as week, 'T2_CH_CERN' as sitename, fullname as collName, numAccesses, procTime, readBytes;
aggr   = GROUP select BY (week, sitename, collName);
result = FOREACH aggr GENERATE FLATTEN(group), SUM(select.numAccesses) as numAccesses, SUM(select.procTime) as totCPU, SUM(select.readBytes) as readBytes;
