...
result = FOREACH aggr4 GENERATE FLATTEN(group), SUM(select1.readBytes) as readBytes, SUM(select1.numAccesses) as numAccesses, SUM(select1.procTime) as totCPU, COUNT(select1.server_username) as numUsers;
